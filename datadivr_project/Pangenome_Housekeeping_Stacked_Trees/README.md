...
Native DataDiVR files are in the project root plus the `layouts`, `layoutsl`, `layoutsRGB`, `links`, `linksRGB`, and `nodesizes` directories. The eight presentation stages are pre-rendered into scene-specific texture files in those directories.
...
- `pfile.json` includes `subtreeHighlightAnimation.presentation`, an 8-stage sequence whose node and link colors are already compiled into native DataDiVR scene textures: all layers, GAPDH, ENO1, RPLP0, the Ray-finned fish subtree in each layer, and finally the full Ray-finned fish subtree plus its inter-layer connections.
